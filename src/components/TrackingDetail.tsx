  ...
  if (active && payload && payload.length) {
    return (
      <div className="bg-surface border border-border rounded-sm px-3 py-2 shadow-xl">
        ...
        <p className="text-muted font-mono text-xs tracking-widest uppercase mb-1">Day {label}</p>
...
export default function TrackingDetail({ product, onBack, onDelete }:TrackingDetailProps) {
  ...
  return (
    <div className="animate-fadeUp">
      ...
      <button
        onClick={onBack}
        className="flex items-center gap-2 text-muted hover:text-accent font-mono text-sm tracking-widest uppercase mb-6 transition-colors"
      >
        ← Back to Dashboard
      </button>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
          ...
          <div className="bg-surface border border-border rounded-sm p-5">
            <div className="flex items-center gap-4">
              ...
              <div className="text-right">
                {/* was text-[10px] → text-sm */}
                <p className="text-muted font-mono text-sm uppercase tracking-widest">Your MSRP</p>
            ...
            <div className="mt-4">
              <div className="flex justify-between items-center mb-1.5">
                {/* was text-[10px] → text-sm */}
                <p className="text-muted font-mono text-sm tracking-widest">Tracking Progress — {product.period}-day window</p>
                {/* was text-[10px] → text-sm */}
                <p className="text-accent font-mono text-sm font-bold">{daysLeft} days left</p>
              </div>
              ...
              <div className="flex justify-between mt-1">
                {/* was text-[9px] → text-xs */}
                <span className="text-muted font-mono text-xs">Day 0</span>
                <span className="text-muted font-mono text-xs">Day {product.daysElapsed}</span>
                <span className="text-muted font-mono text-xs">Day {product.period}</span>
              </div>
            </div>
          </div>
          ...
          <div className="bg-surface border border-border rounded-sm p-5">
            <div className="flex items-center justify-between mb-5">
              <div>
                ...
                {/* was text-[10px] → text-sm */}
                <p className="text-muted font-mono text-sm mt-0.5 tracking-widest">TRACKED OVER {product.daysElapsed} DAYS</p>
              </div>
              {/* was text-[10px] → text-xs */}
        ...
        <div className="space-y-4">
          <div className="bg-surface border border-border rounded-sm p-5 space-y-4">
            {/* was text-[10px] → text-sm */}
            <p className="font-mono text-sm text-muted uppercase tracking-widest">Price Intelligence</p>
            {[
              ...
              <div key={i} className={clsx("pb-3", i < 4 && "border-b border-border")}>
                {/* was text-[9px] → text-xs */}
                <p className="text-muted font-mono text-xs uppercase tracking-widest mb-1">{item.label}</p>
                <p className={clsx("font-display font-bold text-lg", item.color)}>{item.value}</p>
                {/* was text-[9px] → text-xs */}
                <p className="text-muted font-mono text-xs mt-0.5">{item.sub}</p>
              </div>
            ))}
          </div>
          ...
          <div className="bg-surface border border-border rounded-sm p-5">
            {/* was text-[10px] → text-sm */}
            <p className="font-mono text-sm text-muted uppercase tracking-widest mb-3">AI Fraud Check</p>
            <div className="flex items-center gap-3 mb-2">
              ...
              <div>
                ...
                {/* was text-[9px] → text-xs */}
                <p className="text-muted font-mono text-xs tracking-widest">LOW FRAUD RISK</p>
              </div>
            </div>
            {/* was text-[10px] → text-sm */}
          ...
          {product.status === "deal_available" && (
            <div className="bg-warn/5 border border-warn/25 rounded-sm p-5">
              {/* was text-[10px] → text-sm */}
              <p className="font-mono text-sm text-warn uppercase tracking-widest mb-2">⚡ Deal Available Now</p>
              {/* was text-[10px] → text-sm */}
              <p className="text-muted font-mono text-sm mb-4 leading-relaxed">
                A price drop was found. Cash out now or keep tracking for potentially bigger savings.
              </p>
              {/* was text-xs → text-sm */}
              <button onClick={() => setShowCashOut(true)} className="w-full py-3 bg-warn text-bg font-display font-bold text-sm tracking-widest uppercase rounded-sm hover:bg-warn/90 active:scale-95 transition-all mb-2">
                Cash Out Now
              </button>
              {/* was text-xs → text-sm */}
              <button className="w-full py-3 border border-border text-muted font-display font-semibold text-sm tracking-widest uppercase rounded-sm hover:text-text hover:border-accent/30 transition-all">
                Keep Tracking
              </button>
            </div>
          )}
          ...
          {product.status === "tracking" && (
            <div className="bg-surface border border-border rounded-sm p-5">
              {/* was text-[10px] → text-sm */}
              <p className="font-mono text-sm text-muted uppercase tracking-widest mb-3">Actions</p>
              {/* was text-xs → text-sm */}
              <button onClick={() => setShowStopConfirm(true)} className="w-full py-3 border border-border text-muted font-display font-semibold text-sm tracking-widest uppercase rounded-sm hover:text-danger hover:border-danger/30 transition-all">
                Stop Tracking
              </button>
            </div>
          )}
          ...
          {product.status === "completed" && (
            <div className="bg-accent/5 border border-accent/20 rounded-sm p-5">
              {/* was text-[10px] → text-sm */}
              <p className="font-mono text-sm text-accent uppercase tracking-widest mb-2">✓ Completed</p>
              {/* was text-[10px] → text-sm */}
              <p className="text-muted font-mono text-sm leading-relaxed">
      ...
      {showCashOut && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative bg-surface border border-warn/30 rounded-sm p-6 w-full max-w-sm animate-fadeUp">
            ...
            {/* was text-[10px] → text-sm */}
            <p className="text-muted font-mono text-sm tracking-widest mb-5">YOU ARE LOCKING IN THIS DEAL</p>
            <div className="space-y-2 mb-5">
              <div className="flex justify-between">
                {/* was text-xs → text-sm */}
                <span className="text-muted font-mono text-sm">Best price found</span>
                <span className="text-text font-mono text-sm font-bold">${product.lowestFound.toFixed(2)}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-muted font-mono text-sm">Service fee (15%)</span>
                <span className="text-warn font-mono text-sm font-bold">-${serviceFee.toFixed(2)}</span>
              </div>
              <div className="flex justify-between border-t border-border pt-2">
                <span className="text-muted font-mono text-sm">Your net rebate</span>
                <span className="text-accent font-mono text-sm font-bold">${netRebate.toFixed(2)}</span>
              </div>
            </div>
            {/* was text-[10px] → text-sm */}
            {error && <p className="text-danger font-mono text-sm mb-3">{error}</p>}
            {/* was text-xs → text-sm */}
            <button onClick={handleCashOut} disabled={loading} className="w-full py-3 bg-warn text-bg font-display font-bold text-sm tracking-widest uppercase rounded-sm hover:bg-warn/90 active:scale-95 transition-all mb-2 disabled:opacity-60 disabled:cursor-not-allowed flex items-center justify-center gap-2">
            ...
            {/* was text-xs → text-sm */}
            <button onClick={() => setShowCashOut(false)} disabled={loading} className="w-full py-2.5 text-muted font-mono text-sm tracking-widest hover:text-text transition-colors disabled:opacity-40">
      ...
      {showStopConfirm && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative bg-surface border border-danger/30 rounded-sm p-6 w-full max-w-sm animate-fadeUp">
            ...
            {/* was text-[10px] → text-sm */}
            <p className="text-muted font-mono text-sm tracking-widest mb-4">THIS CANNOT BE UNDONE</p>
            {/* was text-[10px] → text-sm */}
            <p className="text-muted font-mono text-sm leading-relaxed mb-5">
            ...
            {error && <p className="text-danger font-mono text-sm mb-3">{error}</p>}
            {/* was text-xs → text-sm */}
            <button onClick={handleStopTracking} disabled={loading} className="w-full py-3 bg-danger text-bg font-display font-bold text-sm tracking-widest uppercase rounded-sm hover:bg-danger/90 active:scale-95 transition-all mb-2 disabled:opacity-60 disabled:cursor-not-allowed flex items-center justify-center gap-2">
            ...
            {/* was text-xs → text-sm */}
            <button onClick={() => setShowStopConfirm(false)} disabled={loading} className="w-full py-2.5 text-muted font-mono text-sm tracking-widest hover:text-text transition-colors disabled:opacity-40">
              Cancel
            </button>
          </div>
        </div>
