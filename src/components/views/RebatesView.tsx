  ...
  return (
    <div className="animate-fadeUp">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-3 mb-8">
        {[
          ...
          <div key={i} className="bg-surface border border-border rounded-sm px-5 py-4">
            {/* was text-[10px] → text-sm */}
            <p className="text-muted font-mono text-sm uppercase tracking-widest mb-1">{s.label}</p>
            <p className={clsx("font-display font-extrabold text-2xl", s.color)}>{s.value}</p>
            {/* was text-[10px] → text-sm */}
            <p className="text-muted font-mono text-sm mt-0.5">{s.sub}</p>
          </div>
        ))}
      </div>
      ...
      <div className="flex items-center gap-4 mb-4">
        {/* was text-[10px] → text-sm */}
        <p className="font-mono text-sm text-muted tracking-widest uppercase">Rebate History</p>
      ...
      {loading && (
        <div className="flex items-center justify-center h-48">
          <div className="flex flex-col items-center gap-4">
            ...
            {/* was text-[10px] → text-sm */}
            <p className="text-muted font-mono text-sm tracking-widest uppercase">Loading rebates...</p>
          </div>
        </div>
      )}
      ...
      {!loading && !error && rebates.length === 0 && (
        <div className="flex flex-col items-center justify-center h-48 border border-dashed border-border rounded-sm">
          ...
          <p className="text-muted font-display font-bold text-base mt-2">No rebates yet</p>
          {/* was text-[10px] → text-sm */}
          <p className="text-muted font-mono text-sm mt-1 tracking-widest">Cash out a deal to earn your first rebate</p>
        </div>
      )}
            ...
            return (
              <div key={r.id} className={clsx("flex items-center gap-4 px-5 py-4", i < rebates.length - 1 && "border-b border-border")}>
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="font-display font-bold text-text text-base truncate">Product #{r.productId.slice(0, 8)}</p>
                  {/* was text-[10px] → text-sm */}
                  <p className="text-muted font-mono text-sm mt-0.5">{date}</p>
                </div>
                <div className="hidden lg:flex items-center gap-6 text-right">
                  <div>
                    {/* was text-[9px] → text-sm */}
                    <p className="text-muted font-mono text-sm uppercase tracking-widest">Gross Save</p>
                    {/* was text-xs → text-base */}
                    <p className="text-text font-mono text-base font-bold">${r.gross.toFixed(2)}</p>
                  </div>
                  <div>
                    {/* was text-[9px] → text-sm */}
                    <p className="text-muted font-mono text-sm uppercase tracking-widest">Fee (15%)</p>
                    {/* was text-xs → text-base */}
                    <p className="text-danger font-mono text-base font-bold">-${r.fee.toFixed(2)}</p>
                  </div>
                </div>
                <div className="text-right">
                  {/* was text-[9px] → text-sm */}
                  <p className="text-muted font-mono text-sm uppercase tracking-widest">Net Rebate</p>
                  {/* kept text-base (already good size) */}
                  <p className="text-accent font-display font-bold text-base">${r.net.toFixed(2)}</p>
                </div>
                {/* was text-[10px] → text-sm */}
                <div className={clsx("px-2 py-1 rounded-sm border text-sm font-mono tracking-widest flex-shrink-0", st.color, st.bg, st.border)}>
                  {st.label}
                </div>
              </div>
            );
