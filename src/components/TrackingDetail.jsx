...
function generatePriceHistory(msrp, daysElapsed, lowestFound) {
  ...
  // Make sure lowest is represented
...
export default function TrackingDetail({ product, onBack }) {
    ...
    <div className="animate-fadeUp">
            ...
            <div className="flex items-center justify-between mb-5">
              ...
              <div className="flex items-center gap-4 text-[10px] font-mono">
                <span className="flex items-center gap-1.5 text-accent">
                  <span className="w-4 h-0.5 bg-accent inline-block" /> Price
                </span>
                <span className="flex items-center gap-1.5 text-muted">
                  <span className="w-4 h-px bg-muted border-t border-dashed border-muted inline-block" /> MSRP
                </span>
              </div>
            </div>
            <div style={{ height: 240 }}>
              <ResponsiveContainer width="100%" height="100%">
                <LineChart data={priceHistory} margin={{ top: 4, right: 4, bottom: 0, left: 0 }}>
                  ...
                  <XAxis
                    dataKey="day"
                    tick={{ fill: "#3A4A5C", fontSize: 9, fontFamily: "Space Mono" }}
                    tickLine={false}
                    axisLine={{ stroke: "#1A2030" }}
                    tickFormatter={(v) => `D${v}`}
                    interval={Math.floor(product.daysElapsed / 6)}
                  />
                  <YAxis
                    tick={{ fill: "#3A4A5C", fontSize: 9, fontFamily: "Space Mono" }}
                    tickLine={false}
                  ...
                  <Line
                    ...
                    strokeWidth={2}
                    dot={false}
                    activeDot={{ r: 4, fill: "#00FF87", strokeWidth: 0 }}
                  />
                </LineChart>
              </ResponsiveContainer>
            </div>
        ...
        <div className="space-y-4">
          ...
          {product.status === "deal_available" && (
            <div className="bg-warn/5 border border-warn/25 rounded-sm p-5">
              ...
              <button
                ...
                className="w-full py-3 bg-warn text-bg font-display font-bold text-xs tracking-widest uppercase rounded-sm hover:bg-warn/90 active:scale-95 transition-all mb-2"
              >
                Cash Out Now
              </button>
          ...
          {product.status === "tracking" && (
            <div className="bg-surface border border-border rounded-sm p-5">
              ...
              <button className="w-full py-3 border border-border text-muted font-display font-semibold text-xs tracking-widest uppercase rounded-sm hover:text-danger hover:border-danger/30 transition-all">
                Stop Tracking
              </button>
            </div>
          )}
      ...
      {showCashOut && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          <div className="absolute inset-0 bg-black/70 backdrop-blur-sm" onClick={() => setShowCashOut(false)} />
          <div className="relative bg-surface border border-warn/30 rounded-sm p-6 w-full max-w-sm animate-fadeUp">
            ...
            <div className="space-y-2 mb-5">
              ...
              <div className="flex justify-between">
                <span className="text-muted font-mono text-xs">Your rebate (est.)</span>
            ...
            <button
              onClick={() => setShowCashOut(false)}
              className="w-full py-2.5 text-muted font-mono text-xs tracking-widest hover:text-text transition-colors"
            >
